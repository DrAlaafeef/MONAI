    ...
    for(int dh = -1; dh <= 1; dh++)
    {
        for(int dw = -1; dw <= 1; dw++)
        {
            ...
            if(nh >=0 && nh < height && nw >=0 && nw < width)
            {
                ...
                space_dis = (float)sqrt(dw*dw + dh*dh);
	                        ...
	                        int height, int width, int channel, float lambda, int iteration)
{
    ...
    for(int it = 0; it<iteration; it++)
    {
        ...
        int dh_f[4] = {-1, -1, -1, 0};
        int dw_f[4] = {-1, 0, 1, -1};
        float local_dis_f[4] = {(float)sqrt(2.0), 1.0, (float)sqrt(2.0), 1.0};
        // forward pass
        for(int h = 0; h < height; h++)
        {
            for (int w = 0; w < width; w++)
        ...
        int dh_b[4] = {0, 1, 1, 1};
        int dw_b[4] = {1, -1, 0, 1};
        float local_dis_b[4] = {1.0, (float)sqrt(2.0), 1.0, (float)sqrt(2.0)};
